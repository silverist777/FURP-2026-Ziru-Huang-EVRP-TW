...
def check_solution(solution, depot, customers, config):
    ...
    vehicle_limit_violations = vehicle_limit_violation_count(
        route_count=len(route_metrics), config=config, violations=violations
    )
    ...
    return FeasibilityReport(
        ...
        vehicle_limit_violations=vehicle_limit_violations,
...
def vehicle_limit_violation_count(route_count, config, violations):
    """Adds one global violation when a solution uses too many vehicles."""

    if config.max_vehicles is None or route_count <= config.max_vehicles:
        return 0
    violations.append(
        Violation(
            constraint="vehicle_limit",
            route_index=None,
            location="fleet",
            detail=f"vehicles_used={route_count}, maximum={config.max_vehicles}",
        )
    )
    return 1
...
def print_benchmark_report(report, runtime_seconds=None, seed=None, solver="PyVRP"):
    ...
    print(f"  vehicle_limit_violations={report.vehicle_limit_violations}")
