...
from core.evrptw_fleet_policy import apply_evrptw_vehicle_limit
...
class InstanceData:
    ...
    def checker_config(self):
        return CheckerConfig(
            ...
            max_vehicles=self.vehicle.num_available,
        )
...
def load_instance_data(data, default_name="instance"):
    # EVRPTW experiments share one mandatory, data-derived fleet cap. Applying
    # it here prevents solver-specific CLI overrides from silently expanding
    # the fleet and also records the policy in emitted instance payloads.
