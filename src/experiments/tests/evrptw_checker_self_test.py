...
from checkers.feasibility_checker import (
    CheckerConfig,
    ChargingStationSpec,
    CustomerSpec,
    DepotSpec,
...
from methods.pyvrp.parse_schneider_instance import convert_schneider_instance, write_instance_json


ROOT = Path(__file__).resolve().parents[2]
SAMPLE_TEXT = ROOT / "data" / "schneider_sample.txt"
SAMPLE_JSON = ROOT / "log" / "week2" / "evrptw-pipeline" / "schneider_sample_converted.json"
...
def test_basic_checker_scenarios():
    ...
    fleet_limited = check_explicit_routes(
        routes=[["C1"], ["C2"]],
        depot=depot,
        customers=[close, far],
        charging_stations=[],
        config=replace(config, battery_capacity=None, initial_battery=None, max_vehicles=1),
    )
    assert_report(not fleet_limited.feasible, "Fleet-limit excess should be infeasible.")
    assert_report(
        fleet_limited.vehicle_limit_violations == 1,
        "Expected one global fleet-limit violation.",
    )
